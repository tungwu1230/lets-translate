...
npm run proxy      # legacy zero-dependency local proxy (server.js, port 3001)
```
...
**Important consequence:** under plain `npm run dev` there is no `/api/proxy` route — API calls only work when deployed to Vercel (or via `vercel dev`). `server.js` is an older standalone proxy that only knows OpenAI/Gemini paths; the frontend no longer targets it directly (it can still be used as a "custom" endpoint).
...
- Streaming (settings toggle) parses SSE `data:` lines (OpenAI/custom) or JSON-array chunks (Gemini); `onChunk` receives the *accumulated* text, not the delta.
