  ...
  return (
    <section className="settings-drawer-content" aria-label="歷史紀錄">
      ...
      <div
        ...
        style={{ display: 'flex', flexDirection: 'column', gap: '16px', paddingBottom: '32px' }}
      >
        {records.length === 0 ? (
          <div
            ...
            >
              ...
              >
                <div style={{ fontSize: '12px', fontWeight: 'bold', color: 'var(--accent-color)' }}>
                  {getLanguageLabel(record.sourceLanguage)} →{' '}
                  {getLanguageLabel(record.targetLanguage)}
                </div>
                ...
                >
                  ...
                  {record.usage && (
                    ...
                    >
                      <Coins size={12} />
                      <span>
                        Tokens: {record.usage.totalTokens} (In: {record.usage.promptTokens} / Out:{' '}
                        {record.usage.completionTokens})
                        {record.usage.estimatedCostUsd !== undefined &&
                          ` · 估算花費: ${formatUsd(record.usage.estimatedCostUsd)}`}
                      </span>
                    </div>
                  )}
                </div>
